...
// Cleanup old messages from cache periodically
setInterval(() => {
  if (processedMessageIds.size > MAX_CACHE_SIZE) {
    // Remove oldest messages (approximate, since Set doesn't have order)
    const messages = Array.from(processedMessageIds);
    const toRemove = messages.slice(0, messages.length - MAX_CACHE_SIZE);
