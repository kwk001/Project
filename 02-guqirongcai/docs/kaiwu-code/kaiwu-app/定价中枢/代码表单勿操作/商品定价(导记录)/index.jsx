class LowcodeComponent extends Component {
      ...
      for (const item of reQuoteList) {
        ...
        for (const e of supplierIds) {
          let obj = {
            ...
            haijuRemarkStr: item.remark, // 海聚方备注预留字段
          }
  ...
  async confirmSubmitReQuoteData() {
    ...
    if (res.code === 200) {
      ...
      if (reQuotePhoneNumber && reQuotePhoneNumber.length > 0) {
        // 【海聚公司】温馨提示：您有标包18（水果）商品须2次议价，请在2024年7月26日（周五）14：00前完成议价。
        ...
        const sendSmsParams = {
          ...
          content: `【海聚公司】温馨提示：您有${curEditData.packageName}商品须2次议价，请在${time1}（${time2}）${time3}前完成议价。`
        }
  ...
  async getDetailTableData(curEditData) {
    ...
    if (tableRes.data) {
      if (tableRes.data.records && tableRes.data.records.length > 0) {
        ...
        const packageSupplierList = await this.queryPackageSupplierList(goodsIdList)
          ...
          if (supplierData === 'RE_QUOTE' || supplierData === 'NOT_CONFIRM') {
            ...
            if (item.finalSupplier && item.finalSupplier.length > 0) {
              ...
              if (this.state.pricingOprType !== 'PRICING_DETAIL' && this.state.pricingOprType !== 'APPROVE') {
                ...
                // 初始化赋值的时候也要同步计算下海聚平台测算价，补税差，考虑税差
                cesuanList.push({
                  goodsCode: item.goodsId,
  ...
  // 获取该商品所对应的标包下的供应商
  async queryPackageSupplierList(goodsIdList) {
    let list = {}
    const searchParams = {
      ids: [...goodsIdList],
    }
  ...
  renderShiTiaoJSX() {
    ...
    const { shiTiaoHistoryList } = this.state
    return (<div>
      <RenderShiTiaoJSXData shiTiaoHistoryList={shiTiaoHistoryList}></RenderShiTiaoJSXData>
    </div>)
  }
  // 查看最新市场价详情
