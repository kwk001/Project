class LowcodeComponent extends Component {
  ...
  renderTableJSX() {
    ...
    const oprRender = (value, index, record) => {
      ...
      if (record.status == '待审批') {
        const appPerson = userApproveList.find(item => Number(item.approveIndex) == Number(record.approveIndex || 1))
        if (appPerson) {
          showDeliveryBtn = appPerson.approvePerson.split(',').includes(currentUser.id)
        }
      }
      return (<div className={'opr-button'}>
        ...
        {record.status == '待审批' && showDeliveryBtn && <Button type="primary"
          style={{ 'margin-right': '8px' }}
        ...
        {record.status == '待审批' && <Button type="primary"
          style={{ 'margin-right': '8px' }}
        ...
        {['审批中', '待生效'].includes(record.status) && <Button type="primary"
          style={{ 'margin-right': '8px' }}
  ...
  queryFormData() {
    ...
    this.setState({
      ...
      this.dataSourceMap.queryProductSupply.load(queryConditions2).then(res => {
        this.setState({
          loading: false
        })
        if (res.code == 1) {
  ...
  async onOk() {
    ...
    const { goodsSupplyForm, userApproveList, customerTypeList, formMap, businessTypePriceMap, goodsOptions, supplierOptions } = this.state
    ...
    console.log(businessTypePriceMap)
    ...
    customerTypeList.forEach(customerType => {
      ...
      if (businessTypePriceMap[key].price) {
  ...
  async openDetailDialog(rowDetail, operType) {
    ...
    console.log("rowDetail.businessTypePrice:", rowDetail.businessTypePrice)
    console.log("businessTypePriceMap:", businessTypePriceMap)
   ...
   * 审核通过或者驳回
   */
  onClickApproval(event, param) {
    let { goodsSupplyForm, formMap, userApproveList } = this.state
    let data = { ...goodsSupplyForm }
    let pass = true
    if (param.status == '通过') {
      if (Array.isArray(userApproveList) && userApproveList?.length) {
    ...
    if (param.status == '通过') {
      if (pass) {
        ...
        goodsSupplyForm.approveIndex = goodsSupplyForm.approveIndex + 1
      }
  ...
  renderGoodsHistoryInfo() {
    ...
    console.log(rowRecord1, rowRecord2, 'rowRecord1, rowRecord2')
