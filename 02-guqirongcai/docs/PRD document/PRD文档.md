...
| 属性 | 描述 |
|------|------|
| **角色定义** | 负责产品码打印和数据录入的生产一线员工 |
| **工作职责** | 根据生产任务打印产品溯源码；录入批次生产和质检信息 |
| **用户特征** | 年龄20-45岁，高中及以上学历，对电脑操作有一定基础 |
| **使用场景** | 在生产车间或仓库，使用工控电脑或平板操作 |
| **使用频率** | 每日多次 |
| **核心诉求** | 操作简单高效、出错率低、打印稳定 |
...
| 属性 | 描述 |
|------|------|
| **触发情境** | 生产车间完成一批羽绒产品的生产和质检后，需要为该产品批次生成溯源码并打印 |
| **用户行为** | 生产操作员登录系统，选择产品档案，填写批次信息，录入质检数据，生成二维码并打印 |
| **期望目的** | 为该批次产品生成唯一的溯源标识，便于后续追溯和查询 |
| **前置条件** | 产品档案已维护；打印设备已配置并连接；操作员有打印权限 |
| **后置结果** | 成功打印溯源标签，批次信息保存到系统，消费者可通过二维码查询 |

**操作流程**：
1. 操作员点击"新增打印"按钮，打开打印表单
...
| **前置条件** | 产品已打印溯源码并录入系统；消费者手机有微信和网络连接 |
...
| 下游模块 | 影响类型 | 影响说明 | 数据流向 |
|---------|---------|---------|---------|
| 产品码打印 | 数据引用 | 产品档案数据用于创建产品批次 | 本模块 → 产品码打印 |

##### 4.3.6.3 模块协同流程图

```text
[产品分类管理] ──数据引用──> [产品档案管理] ──数据引用──> [产品码打印]
                                  ↑
[计量单位管理] ──数据引用───────┘
```
...
| DC2 | 禁用档案不可选 | 产品码打印选择档案时 | 筛选条件增加status=启用 |
...
- **功能名称**：产品码打印
- **功能目标**：生成产品溯源二维码并进行打印，为每批次产品生成唯一溯源标识
- **功能价值**：实现产品批次追溯，为消费者提供扫码查询入口
...
| 角色 | 数据范围 | 说明 |
|------|----------|------|
| 系统管理员 | 全部数据 | 可查看和操作所有打印记录 |
| 生产操作员 | 本人创建的打印记录 | 仅可操作自己打印的批次 |
| 企业管理层 | 全部数据 | 仅可查看和导出 |
...
| 9 | 打印状态 | printStatus | Enum | 是 | 已打印/未打印 |
| 10 | 打印次数 | printCount | Number | 是 | 默认为1 |
...
用于创建新的产品批次，录入质检信息，生成溯源二维码并打印标签。
...
| 11 | 打印状态 | printStatus | 只读展示 |
| 12 | 打印次数 | printCount | 只读展示 |
...
| **入口形式** | 图标按钮 |
| **入口文案** | 补打 |
| **显示条件** | 用户有补打权限且打印次数<3时显示 |

###### 4.4.7.3.3 业务规则

| 规则编号 | 规则名称 | 规则描述 | 优先级 |
|---------|---------|---------|--------|
| BR1 | 补打次数限制 | 最多补打3次（总打印次数≤4） | 必须 |
| BR2 | 打印机状态 | 补打前检查打印机在线状态 | 必须 |
| BR3 | 打印次数更新 | 补打成功后打印次数+1 | 必须 |

###### 4.4.7.3.4 操作逻辑

| 元素 | 类型 | 触发条件 | 行为描述 | 反馈方式 |
|------|------|---------|---------|---------|
| [补打]按钮 | 图标按钮 | 点击 | 弹出确认对话框 | 确认框提示"确定补打吗？" |
| [确认]按钮 | 按钮 | 点击 | 检查打印机→执行打印→更新次数 | Toast提示"补打成功" |
| [取消]按钮 | 按钮 | 点击 | 关闭确认框 | - |
...
| 13 | 打印状态 | printStatus | - | String | 10 | 是 | 未打印 | - | 已打印/未打印 |
| 14 | 打印次数 | printCount | - | Number | - | 是 | 0 | - | - |
...
| 打印记录 | MES_dayinjilu | 一对多 | batchNo → batchNo | 批次的打印历史 |

**业务规则**：
1. 批次号全局唯一，不可重复
2. 批次号长度4-30字符，建议格式BC+年月日+流水号
3. 生产日期不能晚于今天
4. 打印次数最多4次（首次打印+3次补打）
...
│  数据列表                                                                │
│  ┌──┬──────────┬──────────┬────────┬──────────┬──────────┬──────────┬──────────┐  │
│  │☐ │ [批次号]    │ [产品名称]  │ [数量]  │ [生产日期]  │ [绒子含量]  │ [打印状态]  │ 操作     │  │
│  ├──┼──────────┼──────────┼────────┼──────────┼──────────┼──────────┼──────────┤  │
│  │☐ │ BC2024001│ 95%白鹅绒 │ 500    │ 2024-01-15│ 95%      │ 已打印    │查看 补打│  │
│  │☐ │ BC2024002│ 90%白鹅绒 │ 300    │ 2024-01-16│ 90%      │ 已打印    │查看 补打│  │
│  └──┴──────────┴──────────┴────────┴──────────┴──────────┴──────────┴──────────┘  │
│  共 10 条                    分页: [<] 1 2 ... [>]    每页: [20条 ▼]                │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 并发编辑 | 乐观锁版本号 | 提示刷新后重试 | "数据已被他人修改" |
| 重复提交 | 按钮loading+防抖 | 禁用按钮直到请求完成 | 按钮显示loading状态 |
| 补打次数超限 | 打印次数检查 | 阻止补打 | "已达到最大补打次数" |
| 打印机离线 | 设备状态检测 | 提示设备异常 | "打印机离线，请检查设备连接" |
| 打印缺纸 | 设备状态检测 | 提示设备异常 | "打印机缺纸，请补充纸张" |
...
const dictData = {
  PRINT_STATUS: [
    { value: "已打印", label: "已打印", color: "#52c41a" },
    { value: "未打印", label: "未打印", color: "#d9d9d9" }
  ]
};
```
...
| 上游模块 | 依赖类型 | 依赖说明 | 数据流向 |
|---------|---------|---------|---------|
| 产品码打印 | 数据引用 | 依赖产品码打印产生的批次数据 | 产品码打印 → 本模块 |
...
| 删除已使用数据 | 业务校验 | 阻止删除 | "该数据已被使用，无法删除" |
| 批次号重复 | 提交前校验 | 阻止提交 | "批次号已存在，请更换" |
| 打印机离线 | 状态检测 | 提示设备异常 | "打印机离线，请检查设备连接" |
| 打印缺纸 | 状态检测 | 提示设备异常 | "打印机缺纸，请补充纸张" |
...
| 产品码打印流程 | 验证打印流程完整，数据保存正确，二维码生成正常 | P0 |
| 扫码溯源流程 | 验证扫码解析正确，信息展示完整，深色模式适配 | P0 |
| 补打功能 | 验证补打次数限制，打印记录更新正确 | P1 |
