...
| 3 | 标准名称 | standardName | String | 是 | 如：羽绒服装 |
| 4 | 标准类别 | standardCategory | String | 是 | 国家标准/行业标准/企业标准 |
| 5 | 发布日期 | publishDate | Date | 是 | - |
...
| 2 | 标准名称 | standardName | 输入框 | 是 | 长度2-100 |
| 3 | 标准类别 | standardCategory | 下拉选择 | 是 | 国家标准/行业标准/企业标准 |
| 4 | 发布日期 | publishDate | 日期选择 | 是 | 不能晚于今天 |
| 5 | 状态 | status | 开关 | 是 | 启用/禁用，默认启用 |
...
| 3 | 标准名称 | standardName | 输入框 | String | 100 | 是 | - | - |
| 4 | 标准类别 | standardCategory | 下拉选择 | String | 20 | 是 | - | - |
| 5 | 发布日期 | publishDate | 日期选择 | Date | - | 是 | - | - |
...
**企业信息区域**（消费者扫码溯源时展示的企业信息）：

| 序号 | 字段名称 | 字段标识 | 控件类型 | 是否必填 | 验证规则 | 默认值 | 联动规则 |
|------|---------|---------|---------|---------|---------|--------|---------|
| 1 | 企业名称 | companyName | 输入框 | 是 | 长度2-100字符 | - | - |
| 2 | 企业地址 | companyAddress | 输入框 | 是 | 长度5-200字符 | - | - |
| 3 | 生产许可证 | licenseNo | 输入框 | 是 | 长度10-50字符 | - | - |
| 4 | 质检员 | inspector | 输入框 | 是 | 长度2-20字符 | - | - |
| 5 | 微信公众号图片 | wechatQrImage | 图片上传(Image) | 否 | 支持JPG/PNG，建议300×300像素 | - | - |
| 6 | 企业链接 | companyLinks | 多链接输入 | 否 | 最多5个链接，每个长度≤200字符 | - | 支持官网、电商平台等链接 |
...
| 元素 | 类型 | 触发条件 | 行为描述 | 反馈方式 |
|------|------|---------|---------|---------|
| [编辑]按钮 | 图标按钮 | 点击 | 打开编辑对话框，加载批次数据 | 对话框从中心弹出，填充原数据 |
| [保存]按钮 | 按钮 | 点击 | 校验并保存修改后的数据 | Toast提示"保存成功" |
| [取消]按钮 | 按钮 | 点击 | 关闭对话框，不保存修改 | 如有修改，确认框提示 |

###### 3.4.7.2.5 表单字段

**基本信息区域**（批次号、产品档案、产品名称、产品编号、计量单位、规格型号为只读）：

| 序号 | 字段名称 | 字段标识 | 控件类型 | 是否必填 | 验证规则 | 默认值 | 联动规则 |
|------|---------|---------|---------|---------|---------|--------|---------|
| 1 | 批次号 | batchNo | 输入框(只读) | - | - | 原值 | 不可修改 |
| 2 | 产品档案 | materialId | 输入框(只读) | - | - | 原值 | 不可修改 |
| 3 | 产品名称 | materialName | 输入框(只读) | - | - | 原值 | - |
| 4 | 产品编号 | materialCode | 输入框(只读) | - | - | 原值 | - |
| 5 | 计量单位 | unitName | 输入框(只读) | - | - | 原值 | - |
| 6 | 规格型号 | specification | 输入框(只读) | - | - | 原值 | - |
| 7 | 数量 | quantity | 数字框(Number) | 是 | >0，最大999999 | 原值 | - |
| 8 | 生产日期 | produceDate | 日期选择(DatePicker) | 是 | 不能晚于今天 | 原值 | - |
| 9 | 执行标准 | executionStandard | 下拉选择(Select) | 是 | - | 原值 | 关联执行标准管理 |
| 10 | 产品标准类别 | productStandardCategory | 下拉选择 | 是 | 国标/行标/企标 | 原值 | - |
| 11 | 生产年度 | productionYear | 年份选择(YearPicker) | 是 | 4位年份 | 原值 | - |

**媒体信息区域**：

| 序号 | 字段名称 | 字段标识 | 控件类型 | 是否必填 | 验证规则 | 默认值 | 联动规则 |
|------|---------|---------|---------|---------|---------|--------|---------|
| 1 | 产品视频 | productVideo | 文件上传(Video) | 否 | 支持MP4，最大100MB | 原值 | - |
| 2 | 认证信息 | certificationImages | 图片上传(Image) | 否 | 支持JPG/PNG，最多5张 | 原值 | - |

**质检信息区域**：

| 序号 | 字段名称 | 字段标识 | 控件类型 | 是否必填 | 验证规则 | 默认值 | 联动规则 |
|------|---------|---------|---------|---------|---------|--------|---------|
| 1 | 绒子含量 | downContent | 输入框 | 是 | 如：90%、95% | 原值 | - |
| 2 | 蓬松度 | fluffiness | 输入框 | 是 | 如：850、900+ | 原值 | - |
| 3 | 浊度 | turbidity | 输入框 | 是 | 如：10mm | 原值 | - |
| 4 | 气味 | odor | 输入框 | 是 | 如：无异物 | 原值 | - |
...
| 序号 | 字段名称 | 字段标识 | 控件类型 | 是否必填 | 验证规则 | 默认值 | 联动规则 |
|------|---------|---------|---------|---------|---------|--------|---------|
| 12 | 企业名称 | companyName | 输入框 | 是 | 长度2-100字符 | 原值 | - |
| 13 | 企业地址 | companyAddress | 输入框 | 是 | 长度5-200字符 | 原值 | - |
| 14 | 生产许可证 | licenseNo | 输入框 | 是 | 长度10-50字符 | 原值 | - |
| 15 | 质检员 | inspector | 输入框 | 是 | 长度2-20字符 | 原值 | - |
| 16 | 微信公众号图片 | wechatQrImage | 图片上传(Image) | 否 | 支持JPG/PNG | 原值 | - |
| 17 | 企业链接 | companyLinks | 多链接输入 | 否 | 最多5个链接 | 原值 | - |
...
| 元素 | 类型 | 触发条件 | 行为描述 | 反馈方式 |
|------|------|---------|---------|---------|
| [批次号]链接 | 链接 | 点击 | 打开详情弹窗 | 弹窗显示完整信息和产品码 |
| [查看]按钮 | 图标按钮 | 点击 | 打开详情弹窗 | 弹窗显示完整信息和产品码 |
| [关闭]按钮 | 按钮 | 点击 | 关闭详情弹窗 | - |
...
| 元素 | 类型 | 触发条件 | 行为描述 | 反馈方式 |
|------|------|---------|---------|---------|
| [下载产品码]按钮 | 图标按钮 | 点击 | 触发产品码图片下载 | 自动下载文件，Toast提示"下载成功" |
| [批量下载]按钮 | 按钮 | 点击 | 下载选中批次的产品码（ZIP压缩包） | Toast提示"正在打包下载" |
