...
**功能目标**：提供安全、便捷的系统登录认证，支持多种认证方式，保障系统访问安全，实现用户身份识别与权限初始化

**核心功能点**：
1. **用户认证**：用户名密码认证、短信验证码认证、数字证书认证、双因素认证
2. **安全控制**：密码强度校验、登录失败锁定、IP白名单、会话超时管理
3. **验证码机制**：图形验证码、行为验证码、验证码刷新
4. **登录状态管理**：Token生成与刷新、单点登录控制、登录日志记录
...
| status | tinyint | - | 是 | 1 | 状态：1启用 2禁用 3锁定 |
| login_fail_count | int | - | 是 | 0 | 连续登录失败次数 |
| lock_time | datetime | - | 否 | NULL | 锁定时间 |
| last_login_time | datetime | - | 否 | NULL | 最后登录时间 |
| last_login_ip | varchar | 50 | 否 | NULL | 最后登录IP |
| pwd_update_time | datetime | - | 否 | NULL | 密码更新时间 |
| create_time | datetime | - | 是 | 当前时间 | 创建时间 |
| update_time | datetime | - | 是 | 当前时间 | 更新时间 |
| del_flag | tinyint | - | 是 | 0 | 删除标记：0正常 1删除 |
...
| 角色 | 职责描述 |
|------|----------|
| 海关关员 | 代表海关行使监管职责，负责货物查验、审核和处置 |
| 货站操作员 | 在仓库一线执行货物收发、存储、搬运等操作 |
| 调度人员 | 负责车辆、月台、人力等资源调度和任务协调 |
| 仓库管理员 | 负责仓库日常管理、库存监控、作业安排 |
| 客服人员 | 负责客户咨询、投诉处理、工单跟踪 |
| 财务人员 | 负责费用核算、账单管理和财务结算 |
| 管理人员 | 负责整体运营管理、决策分析和团队管理 |
| 系统管理员 | 负责系统配置、用户权限管理和系统维护 |

### 4.3.2 角色权限矩阵

| 功能模块 | 海关关员 | 货站操作员 | 调度人员 | 仓库管理员 | 客服人员 | 财务人员 | 管理人员 | 系统管理员 |
|----------|----------|------------|----------|------------|----------|----------|----------|------------|
| **基础资料** |
| 航班信息查询 | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ | ✅ | ✅ |
| 货物信息查询 | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ |
...
**设计说明：**
- 顶部：页面标题，清晰标识当前功能模块
- 筛选区：包含常用查询条件（输入框、下拉选择、日期范围等）
- 操作按钮区：左侧放置查询/重置/导出等通用操作，右侧放置新增按钮
- 数据表格：展示业务数据列表，关键字段居中或左对齐
- 分页器：位于表格下方，显示总条数和页码导航
...
│  时间信息区                              │
│  ┌──────────────────────────────────┐  │
│  │ 计划时间：XXXX-XX-XX XX:XX        │  │
│  │ 实际时间：XXXX-XX-XX XX:XX        │  │
│  └──────────────────────────────────┘  │
│                                         │
│  状态信息区                              │
│  ┌──────────────────────────────────┐  │
...
**设计说明：**
- 抽屉标题：明确标识详情内容
- 信息分区：按业务逻辑将信息分组展示（基础信息、时间信息、状态信息等）
- 状态标签：使用彩色标签区分不同状态
- 关联信息：展示与该记录关联的业务数据摘要
- 操作日志：展示状态变更历史记录
...
| 模块名称 | 页面类型 | 主要界面元素 | 交互说明 |
|----------|----------|--------------|----------|
| 航班信息管理 | 列表页 | 航班号输入框、航线下拉选择、状态下拉多选、日期范围选择器、数据表格、分页器 | 支持按航班号、航线、状态、日期筛选；表格展示航班号、航线、计划起飞/到达时间、状态、操作列 |
| 航班信息管理 | 表单页 | 两列布局表单：航班号、航线、出发地、目的地、计划起飞/到达时间、机型、货舱容量、最大载重、板位数量、备注 | 新增/编辑航班信息，必填项校验，时间逻辑校验 |
| 航班信息管理 | 详情页 | 基础信息区、时间信息区、状态信息区、关联信息区（已配板位、关联货物数量）、操作日志区 | 右侧抽屉展示，支持查看完整航班信息和操作历史 |
| 物料/货物管理 | 列表页 | 货物编码/名称输入框、货物类型多选框、HS编码输入框、数据表格 | 支持模糊查询货物编码/名称/HS编码，按货物类型筛选 |
| 物料/货物管理 | 表单页 | 货物编码、货物名称、货物类型下拉选择、HS编码输入框、计量单位选择、存储要求选择、包装类型选择、危险品等级（条件显示） | 货物类型选择危险品时显示危险品等级字段，HS编码格式校验 |
| 客户管理 | 列表页 | 客户编码/名称输入框、客户类型/级别多选框、状态下拉选择、数据表格 | 展示客户编码、名称、类型、级别、信用额度、已用额度、状态 |
| 客户管理 | 表单页 | 客户编码、客户名称、客户类型/级别选择、信用额度输入、联系人/电话、结算方式选择、开户银行/账号 | 手机号格式校验，信用额度数值校验 |

#### 4.4.4.2 仓储管理模块界面设计

| 模块名称 | 页面类型 | 主要界面元素 | 交互说明 |
|----------|----------|--------------|----------|
| 仓库基础设置 | 列表页 | 仓库编码/名称输入框、仓库类型选择、状态筛选、树形结构展示（仓库-库区-库位） | 支持三级结构展开/收起，快速定位库位 |
| 仓库基础设置 | 表单页 | 仓库信息（编码、名称、类型、ERP逻辑仓编码）、库区信息（编码、类型）、库位信息（编码、类型、承重/体积限制、货物类型限制） | 根据选择的仓库类型显示不同的配置项 |
| 入库管理 | 列表页 | 运单号输入框、入库状态选择、日期范围选择、数据表格 | 展示入库通知列表，支持按状态筛选 |
| 入库管理 | 表单页 | 运单号、货物件数/重量、存放库位选择、使用容器选择 | 库位选择器支持按货物类型智能推荐 |
| 库内管理-库存查询 | 列表页 | 多维度查询条件（运单号/库位/客户/货物类型）、库存状态筛选、数据表格 | 展示当前数量、锁定数量、可用数量 |
| 库内管理-移位管理 | 表单页 | 源库位选择、目标库位选择、移位货物列表 | 校验目标库位可用性，支持PDA扫码 |
| 库内管理-盘点管理 | 列表页 | 盘点计划列表、盘点状态筛选 | 支持周期盘点、动碰盘点、全面盘点 |
| 出库管理-打板作业 | 列表页 | 航班号选择、打板状态筛选、数据表格 | 展示打板清单，包含板号、货物明细、重量、体积 |
| 出库管理-打板作业 | 表单页 | 航班号选择、板号输入、货物件数/重量、目的港、打板位选择 | 打板重量体积实时校验，不超航班限额 |

#### 4.4.4.3 车辆调度模块界面设计

| 模块名称 | 页面类型 | 主要界面元素 | 交互说明 |
|----------|----------|--------------|----------|
| 车辆预约 | 列表页 | 车牌号输入框、预约日期选择、业务类型筛选、状态筛选 | 展示预约列表，支持查看预约详情 |
| 车辆预约 | 表单页 | 车牌号输入、预约日期选择（≥当前日期）、预约时段选择、业务类型选择、联系电话、预计货物重量 | 时段可用性校验，预约成功后生成预约码 |
| 排队调度 | 列表页 | 实时排队队列展示、车辆状态标签、优先级标识 | 大屏展示，实时刷新排队状态 |
| 排队调度 | 详情页 | 车辆信息、预约信息、排队序号、分配月台、预计进场时间 | 展示车辆排队详情和调度信息 |
...
| 状态 | 样式描述 | 交互说明 |
|------|----------|----------|
| 默认状态 | 主按钮：蓝色背景（#1890ff），白色文字；次按钮：白色背景，蓝色边框和文字 | 正常可点击状态 |
| 悬浮状态 | 主按钮：深蓝色背景（#40a9ff）；次按钮：浅蓝色背景（#e6f7ff） | 鼠标悬停时显示，提示可点击 |
| 点击状态 | 主按钮：更深蓝色背景（#096dd9）；次按钮：蓝色背景，白色文字 | 鼠标按下时显示 |
| 禁用状态 | 灰色背景（#d9d9d9），灰色文字（#bfbfbf），cursor: not-allowed | 不可点击，操作未满足条件时显示 |
| 加载状态 | 按钮内显示加载动画（Loading图标），禁用点击 | 异步操作进行中，防止重复提交 |

#### 4.4.5.2 表单校验反馈设计

| 校验类型 | 反馈方式 | 设计说明 |
|----------|----------|----------|
| 即时校验 | 输入框失去焦点时校验，错误时输入框边框变红（#ff4d4f），下方显示错误提示文字 | 适用于格式校验（手机号、邮箱等） |
| 提交校验 | 点击提交按钮时校验所有字段，错误字段高亮显示，页面滚动到第一个错误字段 | 适用于必填项校验 |
| 条件校验 | 根据某个字段值动态显示/隐藏其他字段或改变校验规则 | 如货物类型选择危险品时显示危险品等级字段并设为必填 |
| 异步校验 | 输入框后显示校验状态图标（校验中/成功/失败），如唯一性校验 | 如航班号、货物编码的唯一性校验 |
...
#### 4.4.5.3 加载/空状态/错误状态设计

| 状态类型 | 设计规范 | 说明 |
|----------|----------|------|
| 加载状态 | 表格区域显示加载动画（Spin或Skeleton），文字提示"数据加载中..." | 首次加载或筛选后重新加载时显示 |
| 空状态 | 显示空状态插图和文字提示"暂无数据"，可配合"立即新增"按钮 | 查询结果为空时显示 |
| 错误状态 | 显示错误图标和错误信息，提供"重新加载"按钮 | 网络异常或接口错误时显示 |
| 部分加载 | 已加载数据正常显示，新数据区域显示加载动画 | 分页加载或懒加载场景 |

**加载状态详细设计：**

| 场景 | 加载方式 | 提示文字 |
|------|----------|----------|
| 列表页首次加载 | 全屏遮罩+加载动画 | "数据加载中..." |
| 列表页筛选查询 | 表格区域加载动画 | "查询中..." |
| 表单提交 | 提交按钮加载状态 | "保存中..." |
| 分页切换 | 表格区域加载动画 | "加载中..." |
| 导出操作 | 导出按钮加载状态 | "导出中..." |
...
| 场景 | 提示文字 | 建议操作 |
|------|----------|----------|
| 列表无数据 | "暂无数据" | 显示"立即新增"按钮 |
| 筛选结果为空 | "未找到符合条件的数据" | 显示"清除筛选"按钮 |
| 搜索无结果 | "未搜索到相关内容" | 提示更换关键词 |

**错误状态详细设计：**

| 场景 | 错误提示 | 操作建议 |
|------|----------|----------|
| 网络异常 | "网络异常，请检查网络后重试" | 提供"重新加载"按钮 |
| 接口错误 | "服务器繁忙，请稍后重试" | 提供"重新加载"按钮 |
| 权限不足 | "您没有权限访问此页面" | 提示联系管理员 |
| 数据被修改 | "数据已被他人修改，请刷新后重试" | 提供"刷新页面"按钮 |
