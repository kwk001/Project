...
class MyPracticeTasks extends Component {
  constructor() {
    super()
  ...
  static PAGE_FORMCODE = 'page_student_practice_tasks'
  static PAGE_FORMNAME = '我的实训任务'

  state = {
    formMap: { form_practice: '' },
    currentPageConfig: {
      pageFormCode: 'page-student-practice-tasks',
      pageFormName: '我的实训任务',
      projectCode: 'PRO0001',
    },
    dataList: [],
  ...
  async loadData() {
    ...
    try {
      this.setState({
        dataList: [
          { id: '1', taskName: '零件设计实训', courseName: 'CAD/CAM技术', status: 'pending' },
          { id: '2', taskName: '数控编程实训', courseName: '数控加工技术', status: 'in_progress' },
        ],
        total: 2,
      })
    } catch (error) {
      ...
      { title: '所属课程', dataIndex: 'courseName', key: 'courseName' },
      {
        ...
        render: (status) => (
          <Tag color={status === 'completed' ? 'success' : status === 'in_progress' ? 'processing' : 'warning'}>
            {status === 'completed' ? '已完成' : status === 'in_progress' ? '进行中' : '待完成'}
          </Tag>
        ),
      },
      {
    ...
    return (
      <ConfigProvider theme={window.antdTheme} locale={this.getLocale()}>
        <div className="main-container" style={{ padding: 24, background: '#f5f5f5', minHeight: '100vh' }}>
          <h1 style={{ marginBottom: 24 }}>我的实训任务</h1>
          <Card>
            <Table
...
export default MyPracticeTasks
