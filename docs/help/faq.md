...
If you don’t have a global install yet, run it via `pnpm openclaw-cn onboard`.
...
`openclaw-cn onboard` is the recommended setup path. In **local mode** it walks you through:
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `openclaw-cn models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `openclaw-cn models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).
...
Steps:
1) Enable the plugin: `openclaw-cn plugins enable google-gemini-cli-auth`
2) Login: `openclaw-cn models auth login --provider google-gemini-cli --set-default`
...
(`~/.openclaw`) and workspace (`~/clawd`) stay untouched.
...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.openclaw/openclaw.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.openclaw/skills` → bundled → `skills.load.extraDirs`. `clawdhub` installs into `./skills` by default, which Clawdbot treats as `<workspace>/skills`.
...
ClawdHub installs into `./skills` under your current directory (or falls back to your configured Clawdbot workspace); Clawdbot treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.openclaw/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawdHub](/tools/clawdhub).
...
Legacy single‑agent path: `~/.openclaw/agent/*` (migrated by `clawdbot doctor`).
...
Clawdbot reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.openclaw/openclaw.json`):
...
- Channel health: `openclaw-cn channels status`
...
`openclaw-cn models status` reports whether **shell env import** is enabled. “Shell env: off”
...
```bash
openclaw-cn directory groups list --channel whatsapp
```
...
- `openclaw-cn models set ...` (updates just model config)
...
- **Confirm where auth profiles live** (new vs legacy paths)
  ...
  - Legacy: `~/.openclaw/agent/*` (migrated by `clawdbot doctor`)
...
- **Sanity‑check model/auth status**
  - Use `openclaw-cn models status` to see configured models and whether providers are authenticated.
...
- **Sync the Claude Code CLI token on the gateway host**
  - Run `openclaw-cn models status` (it loads + syncs Claude Code CLI credentials).
  - If it still says missing: run `claude setup-token` (or `openclaw-cn models auth setup-token --provider anthropic`) and retry.
- **If the token was created on another machine**
  - Paste it into the gateway host with `openclaw-cn models auth paste-token --provider anthropic`.
...
Clawdbot may temporarily skip a profile if it’s in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `openclaw-cn models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
openclaw-cn models auth order set --provider anthropic anthropic:claude-cli anthropic:default
...
```bash
openclaw-cn models auth order set --provider anthropic --agent main anthropic:claude-cli
```
...
- Use `clawdbot --profile <name> …` per instance (auto-creates `~/.openclaw-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `clawdbot --profile <name> gateway install`.

Profiles also suffix service names (`com.openclaw.<profile>`, `clawdbot-gateway-<profile>.service`, `Clawdbot Gateway (<profile>)`).
...
```bash
openclaw-cn channels status
openclaw-cn channels logs --channel telegram
```
...
```bash
openclaw-cn message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
- Default behavior on DM‑capable channels is **pairing**:
  ...
  - Approve with: `openclaw-cn pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `openclaw-cn pairing list <channel>` if a code didn’t arrive.
