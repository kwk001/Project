...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.clawdbot-cn/clawdbot-cn.json. Prefer CLAWDBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.clawdbot-cn/clawdbot-cn.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.clawdbot-cn/clawdbot-cn.json. Restart the gateway to apply."
        }
